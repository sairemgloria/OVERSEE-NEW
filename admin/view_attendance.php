        ...
        <div class="table-responsive">
            ...
            <div class="col-md-12 pt-4">
                <div class="card w-100">
                    <?php
                    // Initialize a flag variable to track if the name has been displayed
                    // $nameDisplayed = false;

                    // $sql = "SELECT employees.*, attendance.EMP_NAME
                    //     FROM employees 
                    //     LEFT JOIN attendance ON attendance.EMP_NAME = CONCAT(employees.FNAME, ' ', employees.MI, ' ', employees.LNAME) 
                    //     WHERE employees.ID = '" . $_GET['q'] . "'";

                    // $query = $conn->query($sql);

                    // while ($emp = $query->fetch_assoc()) {
                    //     // Check if the name has been displayed
                    //     if (!$nameDisplayed) {
                    //         echo '<h5 class="card-header p-3"><i class="material-icons" id="material-icon">info</i> ' . htmlspecialchars($emp["FNAME"] . ' ' . $emp["LNAME"]) . '\'s Attendance</h5>';

                    //         $nameDisplayed = true; // Set the flag to true
                    //     }
                    // }
                    ?>
                    <div class="card-body">
                        <div class="btn-group my-3">
                            <?php if ($row) : ?>
                                <a href="./includes/export_excelfile_attendance.php?export=<?= $row['ID']; ?>" class="btn bg-primary btn-sm text-light" id="export-btn"><i class="material-icons" id="material-icon">file_download</i> &nbsp;EXPORT</a>
                                <a href="#" onclick="confirmDeleteAttendance(event, <?= $row['ID']; ?>);" class="btn bg-danger btn-sm text-light" id="clear-btn"><i class="material-icons" id="material-icon">delete</i> &nbsp;CLEAR</a>
                            <?php endif; ?>
                        </div>
                        <div class="table-responsive">
